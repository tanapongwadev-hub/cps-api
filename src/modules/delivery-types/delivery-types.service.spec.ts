...
describe('DeliveryTypesService', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(DeliveryType), useValue: repo },
        { provide: getDataSourceToken(), useValue: dataSource },
      ],
    }).compile();

    service = moduleRef.get(DeliveryTypesService);
  });
  ...
  describe('create', () => {
    ...
    it('throws ConflictException on duplicate code', async () => {
      ...
      await expect(
        service.create({ code: 'DT-01', nameTh: 'จัดส่งด่วน' } as any, 'u'),
