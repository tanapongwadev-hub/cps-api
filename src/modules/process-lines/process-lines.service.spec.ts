...
describe('ProcessLinesService', () => {
  let service: ProcessLinesService;
  let repo: {
    create: jest.Mock;
    save: jest.Mock;
    findOne: jest.Mock;
  ...
  beforeEach(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        ProcessLinesService,
        { provide: getRepositoryToken(ProcessLine), useValue: repo },
        { provide: getDataSourceToken(), useValue: dataSource },
      ],
    }).compile();

    service = moduleRef.get(ProcessLinesService);
  });

  describe('create', () => {
    it('normalizes code, trims strings, saves audit ids', async () => {
      const result = await service.create(
        { code: ' pl-01 ', nameTh: ' สายประกอบ 1 ' },
        'user-1',
      );
    ...
    it('throws ConflictException on duplicate code', async () => {
      ...
      await expect(
        service.create({ code: 'PL-01', nameTh: 'สายประกอบ 1' } as any, 'u'),
  ...
  describe('update', () => {
    ...
    it('applies update and stamps updatedBy', async () => {
      ...
      const result = await service.update(
        '1',
        { nameTh: 'สายประกอบ 2', updatedAt: '2026-08-04T00:00:00.000Z' },
        'user-2',
      );
      expect(result.nameTh).toBe('สายประกอบ 2');
