...
describe('ProcessLinesController', () => {
  const service = {
    findAll: jest.fn(),
    findOne: jest.fn(),
    create: jest.fn(),
  ...
  const controller = new ProcessLinesController(
    service as unknown as ProcessLinesService,
  );
  ...
  it('declares the approved routes and permission metadata', () => {
    const prototype = ProcessLinesController.prototype;
    const routes = [
      ['findAll', '/', RequestMethod.GET, PROCESS_LINE_PERMISSIONS.VIEW],
      ['findOne', ':id', RequestMethod.GET, PROCESS_LINE_PERMISSIONS.VIEW],
      ['create', '/', RequestMethod.POST, PROCESS_LINE_PERMISSIONS.CREATE],
      ['update', ':id', RequestMethod.PATCH, PROCESS_LINE_PERMISSIONS.UPDATE],
      [
        'deactivate',
        ':id',
        RequestMethod.DELETE,
        PROCESS_LINE_PERMISSIONS.DELETE,
      ],
      [
        'restore',
        ':id/restore',
        RequestMethod.PATCH,
        PROCESS_LINE_PERMISSIONS.UPDATE,
      ],
    ] as const;

    expect(Reflect.getMetadata(PATH_METADATA, ProcessLinesController)).toBe(
      'process-lines',
    );
  ...
  it('guards every route with auth, active assignment, and permission checks', () => {
    expect(
      Reflect.getMetadata(GUARDS_METADATA, ProcessLinesController),
  ...
  it('forwards route parameters, DTOs, and current user IDs', async () => {
    ...
    const createDto = { code: 'PL-01', nameTh: 'สายประกอบ 1' } as any;
    const updateDto = {
      nameTh: 'x',
