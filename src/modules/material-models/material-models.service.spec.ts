...
function makeQueryBuilder(rows: ModelRow[] = [], total = rows.length) {
  const builder: any = {
...
describe('MaterialModelsService', () => {
  let service: MaterialModelsService;
  let repo: {
    create: jest.Mock;
    save: jest.Mock;
    findOne: jest.Mock;
  ...
  beforeEach(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [
        MaterialModelsService,
        { provide: getRepositoryToken(MaterialModel), useValue: repo },
        { provide: getDataSourceToken(), useValue: dataSource },
      ],
    }).compile();

    service = moduleRef.get(MaterialModelsService);
  });
  ...
  describe('create', () => {
    ...
    it('throws ConflictException on duplicate code', async () => {
      ...
      await expect(
        service.create({ code: 'MD-01', nameTh: 'รุ่น A' } as any, 'u'),
