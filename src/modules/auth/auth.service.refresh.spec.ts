...
function harness() {
  ...
  const config = {
    getOrThrow: (key: string) => key === 'JWT_REFRESH_SECRET' ? 'refresh-test-secret' : 'access-test-secret',
    get: (_key: string, fallback: unknown) => fallback,
  } as unknown as ConfigService;
  const claims = {
    sub: '1', sessionId: '2', userDepartmentRoleId: null, departmentId: null,
    roleCode: null as RoleCode | null, permissionVersion: 1,
  };
  const token = jwt.sign(claims, { secret: 'refresh-test-secret', expiresIn: '7d' });
  const session = {
    id: '2', userId: '1', activeUserDepartmentRoleId: null,
    refreshTokenHash: hash(token), previousRefreshTokenHash: null,
    refreshGraceUntil: null, refreshClaims: null, revokedAt: null,
    expiresAt: new Date((jwt.decode(token) as { exp: number }).exp * 1000),
  } as unknown as AuthSession;
  const user = { id: '1', isActive: true, isLocked: false, permissionVersion: 1 };
  const sessions = {
    ...
    save: jest.fn(async (value: AuthSession) => {
      ...
      if (value.refreshClaims) {
        value.refreshClaims = Object.fromEntries(Object.entries(value.refreshClaims).reverse());
      }
      return value;
    }),
    update: jest.fn(async (_where: unknown, values: Partial<AuthSession>) => Object.assign(session, values)),
  };
  const manager = {
    getRepository: (entity: unknown) => entity === AuthSession ? sessions
      : entity === User ? { findOne: async () => user } : { findOne: async () => null },
  };
  ...
  jest.spyOn(tokens, 'hashRefreshToken').mockImplementation(async (value) => hash(value));
  jest.spyOn(tokens, 'compareRefreshToken').mockImplementation(async (value, stored) => hash(value) === stored);
  const service = Object.assign(Object.create(AuthService.prototype) as AuthService, {
    jwtService: jwt, configService: config, tokenService: tokens,
    authSessionRepository: repository,
    buildAuthenticationResponse: jest.fn(async (_user: unknown, _assignment: unknown, accessToken: string, refreshToken: string) => ({
      success: true, data: { authentication: { accessToken, refreshToken } },
    })),
  });
...
describe('Refresh token lifecycle', () => {
  ...
  it('rotates without replacing the session and preserves the absolute expiry', async () => {
    ...
    expect(h.jwt.verify(tokens.accessToken, { secret: 'access-test-secret' })).toMatchObject({ sessionId: '2' });
    expect(h.jwt.verify(tokens.refreshToken, { secret: 'refresh-test-secret' })).toMatchObject({ sessionId: '2' });
    ...
    expect(h.sessions.findOne).toHaveBeenCalledWith(expect.objectContaining({ lock: { mode: 'pessimistic_write' } }));
  });

  it('ten concurrent tabs receive the same successor token', async () => {
    const h = harness();
    const results = await Promise.all(Array.from({ length: 10 }, () => h.service.refreshToken(h.token)));
    expect(new Set(results.map((r) => r.data.authentication.refreshToken)).size).toBe(1);
    expect(h.session.revokedAt).toBeNull();
    ...
    expect(again.data.authentication.refreshToken).toBe(results[0].data.authentication.refreshToken);
  });
  ...
  it('revokes reuse outside the grace period, and commits that revocation', async () => {
    ...
    expect(h.sessions.save).toHaveBeenLastCalledWith(expect.objectContaining({ revokedAt: expect.any(Date) }));
  });

  it('rejects an expired refresh token before accessing a session', async () => {
    const h = harness();
    const expired = h.jwt.sign({ sub: '1', sessionId: '2' }, { secret: 'refresh-test-secret', expiresIn: -1 });
  ...
  it('refreshes throughout an hour while keeping the original session deadline', async () => {
    ...
    for (let minute = 14; minute <= 70; minute += 14) {
      ...
      expect(h.jwt.verify(response.data.authentication.accessToken, { secret: 'access-test-secret' }).sessionId).toBe('2');
      expect(h.session.revokedAt).toBeNull();
    }
