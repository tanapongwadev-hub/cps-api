...
describe('PermissionGuard', () => {
  ...
  it('allows a user with every required permission', async () => {
    ...
    const service = {
      getEffectivePermissionCodes: jest.fn().mockResolvedValue(['user.create']),
    } as any;
  ...
  it('denies a user without the required permission', async () => {
    ...
    const service = {
      getEffectivePermissionCodes: jest.fn().mockResolvedValue(['user.view']),
    } as any;
    const guard = new PermissionGuard(reflector, service);
  ...
  it('bypasses effective permission lookup for a super admin', async () => {
    ...
    await expect(
      guard.canActivate(context({ id: '1', activeRoleCode: 'SUPER_ADMIN' })),
    ).resolves.toBe(true);
